...
void tape_setmotor( struct machine *oric, int motoron )
{
...
// is found)
static int next_ort_value( unsigned char *ortdata, int *offset, int end, int *anytapsec )
{
  int val=0, skip;
...
static int tapsections( unsigned char *ortbuf, int ortbuflen, unsigned char *scratch, int slow )
{
...
static inline signed short getsmp(unsigned char *buf, int is8bit)
{
...
int wav_convert( struct machine *oric )
{
  // Chunk pointers
  ...
  int stereo = 0, fmtseen = 0, useright = 0;
...
//  fflush(stdout);
  ...
  ortlen = tapsections(ortbuf, ortlen, oric->tapebuf, 0);
  ortlen = tapsections(ortbuf, ortlen, oric->tapebuf, 1);
...
int tape_load_tap( struct machine *oric, char *fname )
{
  FILE *f;
...
void tape_make_fname( char *filename )
{
  int i;

  strcpy( tapefile, tapepath );
  strcat( tapefile, "/" );

  i = strlen(tapefile);
  strncpy( &tapefile[i], filename, 128 );
  tapefile[i+127] = 0;
}
...
void tape_autoinsert( struct machine *oric )
{
  ...
  tape_make_fname( oric->lasttapefile );
  i = strlen(tapefile);
...
/*
      ...
      {
        int justtap = (oric->tsavf != oric->tapecap);
        tape_stop_savepatch( oric );
        if( justtap )
        {
...
//            refreshtape = 1;
    ...
    if( oric->cpu.calcpc == oric->pch_tt_readbyte_pc )
    {
      ...
      oric->cpu.calcop = oric->cpu.read( &oric->cpu, oric->cpu.calcpc );
//      if( oric->tapeoffs >= oric->tapelen ) refreshtape = 1;
    }
  }
}
...
void tape_ticktock( struct machine *oric, int cycles )
{
  ...
  if( ( oric->tapeoffs < 0 ) || ( oric->tapeoffs >= oric->tapelen ) )
  {
    if( oric->tapehitend > 2 )
    {
      ...
      if( ( oric->lasttapefile[0] ) && ( oric->tapeautoinsert ) )
      {
        tape_autoinsert( oric );
        oric->lasttapefile[0] = 0;
      }
