...
struct diskimage
{
  s16   drivenum;              // The drive this disk is inserted into, or -1
  u32   numtracks;             // Number of tracks per side
  u32   numsides;              // Number of sides in the image
  u32   geometry;              // Geometry. I don't know what this is.
  s16   cachedtrack;           // Currently cached track (or -1 for none)
  s16   cachedside;            // Currently cached side (or -1 for none)
  u32   numsectors;            // Number of sectors cached (= number of valid sectors in the current track)
  struct   mfmsector sector[32];  // Cache of pointers to sectors
  u8   *rawimage;              // The raw disk image file loaded into memory
  u32   rawimagelen;           // Size of the raw image file
  int modified;              // Set to TRUE if the image in memory has been modified
  s32   modified_time;         // Cycles since it was last modified
...
struct wd17xx
{
  u8             r_status;          // Status register
  u8             r_track;           // Track register
  u8             r_sector;          // Sector register
  u8             r_data;            // Data register
  u8             c_drive;           // Currently active drive
  u8             c_side;            // Currently active side
  u8             c_track;           // Currently selected track
  u8             c_sector;          // Currently selected sector ID
  u8             sectype;           // When reading a sector, this is used to remember if it was marked as deleted
  int          last_step_in;      // Set to TRUE if the last seek operation stepped the head inwards
...
struct microdisc
{
  u8 status;           // Status register (write to 0x314)
  u8 intrq;            // intrq register (read from 0x314)
  u8 drq;              // drq register (read/write 0x318)
  struct wd17xx *wd;      // Pointer to the WD17xx structure
  struct machine *oric;   // Pointer to the Oric structure
  int diskrom;       // TRUE if the diskrom is enabled
};
...
struct jasmin
{
  u8 olay;              // Overlay RAM enable (0x3fa)
  u8 romdis;            // ROMDIS (0x3fb)
  struct wd17xx *wd;       // Pointer to the WD17xx structure
  struct machine *oric;    // Pointer to the Oric structure
};
